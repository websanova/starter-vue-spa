<script setup lang="ts">
  ...
  const error = computed(() => {
    ...
    return isFailed.value ? i18n.t('features.subscribe.cancel.note_failed') : ''
  })
  ...
  const note = computed(() => {
    ...
    if (isOnTrial.value && endsAt) {
      return i18n.t('features.subscribe.cancel.note_trial', { date: i18n.d(new Date(endsAt), 'short') })
    }

    return i18n.t('features.subscribe.cancel.note')
  })
...
<template>
  <Stack>
    <section>
      <Heading>
        {{ $t('features.heading.subscription_cancel') }}
      </Heading>
      ...
      <Stack gap="sm">
        ...
        <ButtonLoading
          ...
          @click="submit"
        >
          {{ $t('features.subscribe.cancel.submit') }}
        </ButtonLoading>
      </Stack>
    </section>
  </Stack>
