import { computed } from 'vue'
import { usePreferences } from '@/composables/support/usePreferences'
import { useQueryParam } from '@shared/composables/support/useQueryParam'
import { useSettingsStore } from '@shared/stores/settings'

export function useBookmarkParams() {
  const settings = useSettingsStore()

  const preferences = usePreferences()

  const page = useQueryParam('page', { default: 1 })
  const search = useQueryParam<string>('search', { min: settings.data.searchMinLength, replace: true })
  const tagId = useQueryParam<string>('tag_id')

  // Sort is owned by preferences, but changing it still has to land on the
  // first page, which the query params get for free and this does not.
  const sortBy = computed({
    get: () => preferences.bookmarksSortBy.value,
    set: (value) => {
      preferences.bookmarksSortBy.value = value
      page.value = 1
    },
  })

  const sortDir = computed({
    get: () => preferences.bookmarksSortDir.value,
    set: (value) => {
      preferences.bookmarksSortDir.value = value
      page.value = 1
    },
  })

  return { page, search, sortBy, sortDir, tagId }
}

export type BookmarkParams = ReturnType<typeof useBookmarkParams>
