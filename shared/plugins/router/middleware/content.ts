...
/**
 * Delay before revealing freshly navigated content. One paint tick lets
 * the transition start from the loading state instead of snapping straight
 * to the loaded view.
 */
const REVEAL_DELAY = 10

/**
 * Marks each load tier loaded only when it is unchanged across the
 * navigation. A changed tier flips to false so its transition can play;
 * afterEach flips it back to true once the new view paints.
 */
...
/**
 * Reveals the new content after paint and restores scrolling. Skips the
 * delay when the path is unchanged since no tier transition is in flight.
 */
